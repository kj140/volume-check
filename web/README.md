...
1. 住所で地図を移動し、**「敷地の頂点をなぞる」**で敷地の角を順にクリックする
   （最初の点をクリック／ダブルクリック／Enter で閉じる。Backspace で1つ戻る）。
   閉じると各辺の中点に丸が出る。赤＝道路、灰＝隣地で、クリックで切り替える。
   最初に描いた辺が道路の既定。幅員は道路辺に共通。
   軸に揃った矩形でよければ「矩形で描く」でドラッグでもよい（辺は南北・東西を向き、
   **道路の方位**でどちらの辺が間口になるかが決まる）
...
| `geo.py` | 地図の矩形 → 間口・奥行、緯度経度の多角形 → ローカル座標[m]（WGS84 楕円体の弧長）、タイル座標、点の内外判定 |
