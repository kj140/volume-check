...
> **企画検討用の試算です。** 日影規制・高度地区・地区計画は未考慮。
> 天空率は道路斜線・隣地斜線について「緩和できる見込みがあるか」の試算のみ。
...
- 隣地斜線（法56条1項2号）
- **北側斜線（法56条1項3号）** — 真北方向に測る。中高層住専は日影規制の指定があれば適用外
- 絶対高さ制限（法55条）
- 建蔽率による全階一律の絞り込み（法53条）と、防火地域・角地による緩和（法53条3項・6項）
- 外壁後退と斜線後退は**足さず大きいほうを適用**（どちらも境界線からの離れを定めるため）
...
- **天空率による道路斜線・隣地斜線の緩和可能性の判定**（法56条7項1号・2号／令135条の5〜10）— 複数案の比較表にも判定列として出る
- 地図で敷地の角をなぞって（向き・形は任意）、断面図・配置図・各階平面図をブラウザでプレビュー（Web版）
...
skyfactor.py   天空率の算定と、道路斜線・隣地斜線を緩和できるかの判定
drawer.py      DXF出力
main.py        CLI
